...
export default function TekkenGame() {
  ...
  const p1 = searchParams.get("p1"); // e.g., ?p1=jin
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setGameTime((prev) => {
        if (prev <= 1) {
          // Time up → winner by highest health
    ...
    <Suspense fallback={<div className="w-full h-screen flex items-center justify-center bg-black text-white">Loading...</div>}>
      <div className="w-full h-screen bg-black overflow-hidden relative">
        <Canvas shadows camera={{ position: [0, 2, 8], fov: 50 }} gl={{ antialias: true }}>
          <GameScene player1Position={player1Position} player2Position={player2Position} p1={p1} />
        </Canvas>
        ...
        {gameState === "game-over" && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/90 backdrop-blur-sm">
            <div className="text-center space-y-8">
              <h2 className="text-8xl font-bold text-yellow-400 tracking-wider mb-4 animate-pulse">
                VICTORY!
              </h2>
